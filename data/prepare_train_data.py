...
parser.add_argument("--start_id", type = int, default=0, help="memory leak...")
...
def dump_example(n, args):
    ...
    if 'segmentation' in example:
        ...
        imageio.imwrite(segmentation_file, example['segmentation'].astype(np.uint8))
def main():
    ...
    if args.dataset_name == 'cityscapes':
        ...
        data_loader = cityscapes_loader(args.dataset_dir,
                                        ...
                                        img_width=args.img_width,
                                        seq_length=args.seq_length)
    if(args.start_id > data_loader.num_train):
        exit(0)
    for n in range(args.start_id, data_loader.num_train):
        dump_example(n, args)
