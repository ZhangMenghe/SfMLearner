...
for i in ${v_date_list[@]}; do
	...
	python data/prepare_train_data.py --dataset_dir=$v_raw_data_set_path --dataset_name=$v_dataset_name --dump_root=$v_dump_root --seq_length=$v_seq_length --img_width=$v_img_width --img_height=$v_img_height --num_threads=$v_num_threads
done
